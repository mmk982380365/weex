...
public class WXSDKManager {
  ...
  public int getJsEngineType() {
    if (mJsEngineType == 0) {
      synchronized (WXSDKManager.class) {
        if(mJsEngineType == 0) {
          mJsEngineType = getWXJSEngineManager().defaultEngine().engineValue();
          setJsEngineType(mJsEngineType);
        }
      }
    }
    WXLogUtils.e("engine_type","return engine type is " + mJsEngineType);
    return mJsEngineType;
  }

  public void setJsEngineType(int engineType) {
  ...
  public @NonNull IWXJSEngineManager getWXJSEngineManager() {
    if (mWXJSEngineManager == null) {
      synchronized (WXSDKManager.class) {
        if (mWXJSEngineManager == null) {
          mWXJSEngineManager = new DefaultJSEngineManager();
        }
      }
    }

    return mWXJSEngineManager;
  }

  public void setWXJSEngineManager(IWXJSEngineManager engineManager) {
    if (mWXJSEngineManager == null) {
      synchronized (WXSDKManager.class) {
        if (mWXJSEngineManager == null) {
          mWXJSEngineManager = engineManager;
        }
      }
    }
  }
