...
public class WXSDKEngine implements Serializable {
  ...
  private static void doInitInternal(final Application application,final InitConfig config){
    ...
    WXBridgeManager.getInstance().postWithName(new Runnable() {
      ...
      public void run() {
        ...
        WXReactorPluginManager.getInstance().initSo(version,userTrackAdapter);
  ...
  public static boolean registerServiceForNative(String name, String serviceScript, Map<String, Object> options) {
    return WXServiceManager.registerServiceForNative(name, serviceScript, options);
  }
  ...
  public static void setJSCLoader(IWXJSCLoader jscLoader){
    WXSDKManager.getInstance().setJSCLoader(jscLoader);
  }
  ...
  public static class QuickJSBuildTask {
    public String script;
    public QuickJSCompileCallback callback;

    public QuickJSBuildTask(String script, QuickJSCompileCallback callback) {
      this.callback = callback;
      this.script = script;
    }
  }

  public static interface QuickJSCompileCallback {
    public void BuildResult(boolean succeed, byte[] bytecode, int length);
  }

  public static void CompileQuickJSBin(String key, String Script, QuickJSCompileCallback callback) {
    if (TextUtils.isEmpty(key) || TextUtils.isEmpty(Script) || callback == null) {
      return;
    }
