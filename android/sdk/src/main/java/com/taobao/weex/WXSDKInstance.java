...
public class WXSDKInstance implements IWXActivityStateListener,View.OnLayoutChangeListener {
  ...
  public void setHttpCacheAdapter(IHttpCacheAdapter httpCacheAdapter){
    this.mhttpCacheAdapter = httpCacheAdapter;
  }
  public IHttpCacheAdapter getHttpCacheAdapter(){
    return mhttpCacheAdapter;
  }
  ...
  public final WXSDKInstance createNestedInstance(NestedContainer container){
    ...
    if(sdkInstance != null) {
      ...
      IWXJSEngineManager.EngineType engineType = getJSEngineType() == IWXJSEngineManager.EngineType.JavaScriptCore ? IWXJSEngineManager.EngineType.JavaScriptCore
              : IWXJSEngineManager.EngineType.QuickJS;
      sdkInstance.setJSEngineType(engineType);
      sdkInstance.mJSEngineTypeFixed = engineType;
    }
    return sdkInstance;
  }
  ...
  public void preInit(String pageName,
    ...
    //set a flag to preInitMode cause the script can't be predicated
    renderOptions.put("pre_init_mode", "true");
  ...
  private void renderInternal(String pageName,
    ...
    if (isPreInitMode()){
      ...
      WXBridgeManager.getInstance().loadScriptInPreInitModeByteCode(getInstanceId(), template);
    } else {
  ...
  private void renderByUrlInternal(String pageName,
                                   ...
                                   WXRenderStrategy flag) {
    Pair<IWXJSEngineManager.EngineType, String> engineTypeStringPair = WXSDKManager.getInstance().getWXJSEngineManager().engineType(url);
    IWXJSEngineManager.EngineType engineType = engineTypeStringPair.first;
    if ((engineType.engineValue() & WXSDKManager.getInstance().getJsEngineType()) == 0) {
      for (IWXJSEngineManager.EngineType t : IWXJSEngineManager.EngineType.values()) {
        if ((WXSDKManager.getInstance().getJsEngineType() & t.engineValue()) > 0) {
          engineType = t;
          break;
        }
      }
...
//    setRunInMainProcess(WXSDKManager.getInstance().getWXJSEngineManager().runInMainProcess(url));
    ...
    if(engineType == IWXJSEngineManager.EngineType.QuickJSBin) {
      downloadUrl = engineTypeStringPair.second;
    }
    ...
    wxRequest.url = rewriteUri(Uri.parse(downloadUrl),URIAdapter.BUNDLE).toString();
  ...
  public void renderReactor(long context, String template, String data) {
...
//      }
  ...
  public boolean isUsingReactorPage() {
    boolean reactorPageFlag = true;
    IWXConfigAdapter adapter = WXSDKManager.getInstance().getWxConfigAdapter();
    if (adapter != null) {
      String config = adapter.getConfig("android_weex_reactorpage_flag", "isUsingReactorPage", "true");
      reactorPageFlag = Boolean.parseBoolean(config);
    }
  ...
  public IWXJSEngineManager.EngineType getJSEngineType() {
    if(mJSEngineType == null) {
      mJSEngineType = WXSDKManager.getInstance().getWXJSEngineManager().engineType(getBundleUrl()).first;
      if(mJSEngineType != null && mJSEngineTypeFixed != null
              && ((mJSEngineType.engineValue() & mJSEngineTypeFixed.engineValue()) == 0)) {
        mJSEngineType = mJSEngineTypeFixed;
      }
    }
    return mJSEngineType;
  }

  public boolean setJSEngineType(IWXJSEngineManager.EngineType engine_type) {
    if (engine_type == null) {
      return false;
    }
    IWXJSEngineManager.EngineType engineType = engine_type;
    if (mJSEngineTypeFixed != null
            && ((engine_type.engineValue() & mJSEngineTypeFixed.engineValue()) == 0)) {
      engineType = mJSEngineTypeFixed;

    }
    this.mJSEngineType = engineType;

    return this.mJSEngineType == engine_type;
  }
  ...
  public static interface WXProcessNotify {
    void crashed();
    void reboot();
  }
  ...
  public interface UnicornJSInvoker {
    public void invokeCallback(String instanceId,
                               Object... args);
    public void invokeMethod(final String instanceId,
                             String method, final List<Object> params,
                             Object... args);
  }
}
