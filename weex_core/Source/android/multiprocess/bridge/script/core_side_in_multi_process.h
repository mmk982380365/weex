...
class CoreSideInMultiProcess : public WeexCore::ScriptBridge::CoreSide {
 public:
  ...
  void InitIpcClient(int clientFd);
  void HeartBeat(const char* instanceId);

 private:
  std::unique_ptr<WeexIPCClient> ipc_client_;
  std::unique_ptr<weex::base::Thread> ipc_msg_thread_;
