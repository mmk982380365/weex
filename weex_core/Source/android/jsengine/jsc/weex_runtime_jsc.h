...
#ifndef WEEX_PROJECT_WEEX_RUNTIME_JSC_H
#define WEEX_PROJECT_WEEX_RUNTIME_JSC_H

#include "android/multiprocess/weex_ipc_client.h"
#include "core/runtime/weex_runtime.h"

class WeexRuntimeJSC : public WeexRuntime {
 public:

  explicit WeexRuntimeJSC(const std::shared_ptr<WeexCore::ScriptBridge>& script_bridge,
  ...
  int
  exeJS(const std::string &instanceId,
 ...
 private:
  int _initFramework(const std::string &source);

  int _initAppFramework(const std::string &instanceId, const std::string &appFramework);

};

#endif //WEEX_PROJECT_WEEX_RUNTIME_JSC_H
